    "use strict";
    var t = e.i(43476)
      , a = e.i(46932)
      , l = e.i(48148)
      , s = e.i(55487);
    e.i(47167);
    var i = e.i(71645)
      , n = e.i(18566)
      , r = e.i(5427)
      , o = e.i(32252);
    let c = () => (0,
    t.jsxs)("div", {
        className: " h-screen fixed inset-0 z-50 flex flex-col items-center justify-center bg-transparent bg-opacity-40 backdrop-blur-sm",
        children: [(0,
        t.jsx)("div", {
            className: "animate-spin rounded-full h-16 w-16 border-t-4 border-blue-400 border-opacity-60"
        }), (0,
        t.jsx)("div", {
            className: "text-green-700 text-lg my-4 font-medium",
            children: "Checking..."
        })]
    });
    var d = e.i(46577)
      , m = e.i(58152)
      , u = e.i(95187)
      , p = (0,
    u.createServerReference)("401c4f96f9b9e9a798c9215be8fa0a4688deeb8830", u.callServer, void 0, u.findSourceMapURL, "getInitialApplicationData")
      , x = (0,
    u.createServerReference)("70377b6e06ac0b9b4ffe55dbae6a64786750c62482", u.callServer, void 0, u.findSourceMapURL, "submitApplication")
      , f = (0,
    u.createServerReference)("60e622041f4f6b11064a643e7d1efba26d9b1ec7e5", u.callServer, void 0, u.findSourceMapURL, "verifyWebFile")
      , h = e.i(67859)
      , b = e.i(97476);
    let y = [{
        id: 1,
        c_name: "Dhaka",
        prefix: "D"
    }, {
        id: 2,
        c_name: "Chittagong",
        prefix: "C"
    }, {
        id: 3,
        c_name: "Rajshahi",
        prefix: "R"
    }, {
        id: 4,
        c_name: "Sylhet",
        prefix: "S"
    }, {
        id: 5,
        c_name: "Khulna",
        prefix: "K"
    }]
      , v = [{
        id: 2,
        center_info_id: 3,
        ivac_name: "IVAC , RAJSHAHI",
        prefix: "R",
        app_key: "IVACRAJSHAHI",
        visa_fee: "1500.00",
        new_fees_applied_from: "2018-08-05 00:00:00",
        notify_fees_from: "2018-07-29 04:54:32",
        max_notification_count: 2,
        allow_old_amount_until_new_date: 2
    }, {
        id: 3,
        center_info_id: 5,
        ivac_name: "IVAC, KHULNA",
        prefix: "K",
        app_key: "IVACKHULNA",
        visa_fee: "1500.00",
        new_fees_applied_from: "2018-08-05 00:00:00",
        notify_fees_from: "2018-07-29 04:54:32",
        max_notification_count: 2,
        allow_old_amount_until_new_date: 2
    }, {
        id: 4,
        center_info_id: 4,
        ivac_name: "IVAC, SYLHET",
        prefix: "S",
        app_key: "IVACSYLHET",
        visa_fee: "1500.00",
        new_fees_applied_from: "2018-08-05 00:00:00",
        notify_fees_from: "2018-07-29 04:54:32",
        max_notification_count: 2,
        allow_old_amount_until_new_date: 2
    }, {
        id: 5,
        center_info_id: 2,
        ivac_name: "IVAC, CHITTAGONG",
        prefix: "C",
        app_key: "IVACCHITTAGONG",
        visa_fee: "1500.00",
        new_fees_applied_from: "2018-08-05 00:00:00",
        notify_fees_from: "2018-07-29 04:54:32",
        max_notification_count: 2,
        allow_old_amount_until_new_date: 2
    }, {
        id: 7,
        center_info_id: 3,
        ivac_name: "IVAC, RANGPUR",
        prefix: "R",
        app_key: "IVACRANGPUR",
        visa_fee: "1500.00",
        new_fees_applied_from: "2018-08-05 00:00:00",
        notify_fees_from: "2018-07-29 04:54:32",
        max_notification_count: 2,
        allow_old_amount_until_new_date: 2
    }, {
        id: 8,
        center_info_id: 4,
        ivac_name: "IVAC, MYMENSINGH",
        prefix: "S",
        app_key: "IVACMYMENSINGH",
        visa_fee: "1500.00",
        new_fees_applied_from: "2018-08-05 00:00:00",
        notify_fees_from: "2018-07-29 04:54:32",
        max_notification_count: 2,
        allow_old_amount_until_new_date: 2
    }, {
        id: 9,
        center_info_id: 1,
        ivac_name: "IVAC, BARISAL",
        prefix: "D",
        app_key: "IVACBARISAL",
        visa_fee: "1500.00",
        new_fees_applied_from: "2018-08-05 00:00:00",
        notify_fees_from: "2018-07-29 04:54:32",
        max_notification_count: 2,
        allow_old_amount_until_new_date: 2
    }, {
        id: 12,
        center_info_id: 1,
        ivac_name: "IVAC, JESSORE",
        prefix: "D",
        app_key: "IVACJESSORE",
        visa_fee: "1500.00",
        new_fees_applied_from: "2018-08-05 00:00:00",
        notify_fees_from: "2018-07-29 04:54:32",
        max_notification_count: 2,
        allow_old_amount_until_new_date: 2
    }, {
        id: 17,
        center_info_id: 1,
        ivac_name: "IVAC, Dhaka (JFP)",
        prefix: "D",
        app_key: "IVACJFP",
        visa_fee: "1500.00",
        new_fees_applied_from: "2018-08-05 00:00:00",
        notify_fees_from: "2018-07-29 04:54:32",
        max_notification_count: 2,
        allow_old_amount_until_new_date: 2
    }, {
        id: 18,
        center_info_id: 3,
        ivac_name: "IVAC, THAKURGAON",
        prefix: "R",
        app_key: "IVACTHAKURGAON",
        visa_fee: "1500.00",
        new_fees_applied_from: "2018-08-05 00:00:00",
        notify_fees_from: "2018-07-29 04:54:32",
        max_notification_count: 2,
        allow_old_amount_until_new_date: 2
    }, {
        id: 19,
        center_info_id: 3,
        ivac_name: "IVAC, BOGURA",
        prefix: "R",
        app_key: "IVACBOGURA",
        visa_fee: "1500.00",
        new_fees_applied_from: "2018-08-05 00:00:00",
        notify_fees_from: "2018-07-29 04:54:32",
        max_notification_count: 2,
        allow_old_amount_until_new_date: 2
    }, {
        id: 20,
        center_info_id: 1,
        ivac_name: "IVAC, SATKHIRA",
        prefix: "D",
        app_key: "IVACSATKHIRA",
        visa_fee: "1500.00",
        new_fees_applied_from: "2018-08-05 00:00:00",
        notify_fees_from: "2018-07-29 04:54:32",
        max_notification_count: 2,
        allow_old_amount_until_new_date: 2
    }, {
        id: 21,
        center_info_id: 2,
        ivac_name: "IVAC, CUMILLA",
        prefix: "C",
        app_key: "IVACCUMILLA",
        visa_fee: "1500.00",
        new_fees_applied_from: "2018-08-05 00:00:00",
        notify_fees_from: "2018-07-29 04:54:32",
        max_notification_count: 2,
        allow_old_amount_until_new_date: 2
    }, {
        id: 22,
        center_info_id: 2,
        ivac_name: "IVAC, NOAKHALI",
        prefix: "C",
        app_key: "IVACNOAKHALI",
        visa_fee: "1500.00",
        new_fees_applied_from: "2018-08-05 00:00:00",
        notify_fees_from: "2018-07-29 04:54:32",
        max_notification_count: 2,
        allow_old_amount_until_new_date: 2
    }, {
        id: 23,
        center_info_id: 2,
        ivac_name: "IVAC, BRAHMANBARIA",
        prefix: "C",
        app_key: "IVACBRAHMANBARIA",
        visa_fee: "1500.00",
        new_fees_applied_from: "2018-08-05 00:00:00",
        notify_fees_from: "2018-07-29 04:54:32",
        max_notification_count: 2,
        allow_old_amount_until_new_date: 2
    }, {
        id: 24,
        center_info_id: 3,
        ivac_name: "IVAC, KUSHTIA",
        prefix: "R",
        app_key: "IVACKUSHTIA",
        visa_fee: "1500.00",
        new_fees_applied_from: "2023-04-16 13:58:24",
        notify_fees_from: "2023-04-16 13:58:24",
        max_notification_count: 2,
        allow_old_amount_until_new_date: 2
    }]
      , g = [{
        id: 3,
        type_name: "TOURIST VISA"
    }, {
        id: 13,
        type_name: "MEDICAL/MEDICAL ATTENDANT VISA"
    }, {
        id: 1,
        type_name: "BUSINESS VISA"
    }, {
        id: 6,
        type_name: "ENTRY VISA"
    }, {
        id: 19,
        type_name: "DOUBLE ENTRY VISA"
    }, {
        id: 2,
        type_name: "STUDENT VISA"
    }, {
        id: 18,
        type_name: "OTHERS VISA"
    }]
      , _ = (e, t, a, l) => {
        let s = e.message || t
          , i = e.status_code || 422;
        401 === i || 419 === i ? (l({
            message: s,
            type: "error"
        }),
        setTimeout( () => {
            (0,
            m.handleLogout)()
        }
        , 5e3)) : l({
            message: s,
            type: "error"
        })
    }
      , j = () => {
        let e = (0,
        s.useSelector)(e => e.authUser.is_slot_available)
          , m = (0,
        s.useSelector)(e => e.authUser.visa_fee)
          , u = (0,
        s.useSelector)(e => e.ip.clientIp)
          , [j,w] = (0,
        i.useState)(!1)
          , [N,S] = (0,
        i.useState)(null)
          , [k,A] = (0,
        i.useState)(!0)
          , C = (0,
        l.useTranslations)("root")
          , I = (0,
        n.useRouter)()
          , T = (0,
        s.useDispatch)()
          , {applicationData: D} = (0,
        s.useSelector)(e => e.application)
          , R = (0,
        s.useSelector)(e => e.application.isEdit)
          , [F,P] = (0,
        i.useState)(!1)
          , [V,O] = (0,
        i.useState)(null)
          , [E,L] = (0,
        i.useState)([])
          , [U,$] = (0,
        i.useState)("")
          , [M,H] = (0,
        i.useState)("")
          , [B,K] = (0,
        i.useState)("")
          , [W,G] = (0,
        i.useState)(!1)
          , [q,J] = (0,
        i.useState)(v)
          , [Y,z] = (0,
        i.useState)([])
          , [Q,Z] = (0,
        i.useState)("")
          , [X,ee] = (0,
        i.useState)("")
          , [et,ea] = (0,
        i.useState)([])
          , [el,es] = (0,
        i.useState)("")
          , [ei,en] = (0,
        i.useState)("")
          , [er,eo] = (0,
        i.useState)("")
          , [ec,ed] = (0,
        i.useState)(!1)
          , [em,eu] = (0,
        i.useState)("");
        (0,
        i.useEffect)( () => {
            D && ($(D.highcom || ""),
            H(D.webfile_id || ""),
            K(D.webfile_id_repeat || ""),
            en(D.family_count || ""),
            eo(D.visit_purpose || ""),
            es(D.visa_type || ""),
            Z(D.ivac_name || ""),
            ee(D.ivac_id || ""))
        }
        , [D]),
        (0,
        i.useEffect)( () => {
            (async () => {
                A(!0);
                try {
                    let e = await p(u || void 0);
                    e?.status_code === 200 ? (T((0,
                    b.setSlotAvailable)(e.data?.slot_available)),
                    T((0,
                    b.setVisaFee)(e.data?.visa_fee || 1500)),
                    L(y),
                    J(v),
                    z(v),
                    ea(g)) : _(e, "Failed to load initial data", I, O)
                } catch (e) {
                    O({
                        message: "An unexpected error occurred. Please try again later.",
                        type: "error"
                    })
                } finally {
                    A(!1)
                }
            }
            )()
        }
        , []);
        let ep = async () => {
            try {
                ed(!0);
                let e = await f(M, u || void 0);
                200 === e.status_code ? G(!0) : (G(!1),
                _(e, "Failed to verify Web File ID.", I, O))
            } catch (e) {
                O({
                    message: "An unexpected error occurred. Please try again later.",
                    type: "error"
                })
            } finally {
                ed(!1)
            }
        }
        ;
        (0,
        i.useEffect)( () => {
            12 === M.length && 12 === B.length && (M === B ? ep() : O({
                message: "Web File ID does not match",
                type: "error"
            }))
        }
        , [M, B]);
        let ex = "" !== U.trim() && "" !== M.trim() && M.length >= 12 && "" !== B.trim() && B.length >= 12 && W && "" !== Q.trim() && 0 !== m && "" !== el.trim() && "" !== ei.trim() && "" !== er.trim() && er.length >= 15 && "" !== em.trim() && j
          , ef = async () => {
            try {
                P(!0);
                let e = await x({
                    highcom: U,
                    webfile_id: M,
                    webfile_id_repeat: B,
                    ivac_id: X,
                    visa_type: el,
                    family_count: ei,
                    asweoi_erilfs: er,
                    y6e7uk_token_t6d8n3: em
                }, R, u || void 0);
                200 === e.status_code ? (T((0,
                d.setApplicationStep)(2)),
                T((0,
                h.setApplicationData)({
                    highcom: U,
                    webfile_id: M,
                    webfile_id_repeat: B,
                    ivac_id: X,
                    ivac_name: Q,
                    visa_type: el,
                    family_count: ei,
                    visit_purpose: er
                }))) : _(e, "Failed to submit application.", I, O)
            } catch (e) {
                O({
                    message: "An unexpected error occurred. Please try again later.",
                    type: "error"
                })
            } finally {
                P(!1)
            }
        }
        ;
        return ((0,
        i.useEffect)( () => {
            let e = setTimeout( () => {
                w(!0)
            }
            , 5e4);
            return () => clearTimeout(e)
        }
        , []),
        k || ec) ? (0,
        t.jsx)(c, {}) : (0,
        t.jsxs)("div", {
            className: "grid grid-cols-1 gap-x-6 gap-y-2",
            children: [V && (0,
            t.jsx)(o.default, {
                message: V.message,
                type: V.type,
                onClose: () => O(null)
            }), e ? (0,
            t.jsxs)(t.Fragment, {
                children: [(0,
                t.jsxs)("div", {
                    className: "sm:col-span-3",
                    children: [(0,
                    t.jsx)("label", {
                        htmlFor: "high-commission",
                        className: "block text-xs font-light text-black uppercase",
                        children: C("lblSelectHighCommission")
                    }), (0,
                    t.jsx)("div", {
                        className: "mt-2",
                        children: (0,
                        t.jsxs)("select", {
                            id: "center",
                            name: "center",
                            value: U,
                            onChange: e => {
                                let t = e.target.value;
                                $(t),
                                z(q.filter(e => e?.center_info_id == t)),
                                Z("")
                            }
                            ,
                            className: "block w-full rounded-md px-3 py-1.5 text-base font-light text-black outline-1 -outline-offset-1 outline-gray-300 placeholder:text-gray-400 focus:outline-1 sm:text-sm/6",
                            children: [(0,
                            t.jsx)("option", {
                                value: "",
                                disabled: !0,
                                hidden: !0,
                                className: "text-gray-500 font-light",
                                children: C("lblSelectHighCommission")
                            }), E.map(e => (0,
                            t.jsx)("option", {
                                value: e.id,
                                className: "text-black font-light",
                                children: e?.c_name
                            }, e.id))]
                        })
                    })]
                }), (0,
                t.jsxs)("div", {
                    className: "sm:col-span-3",
                    children: [(0,
                    t.jsxs)("label", {
                        htmlFor: "first-name",
                        className: "block text-xs font-light text-black uppercase",
                        children: [C("lblInputWebFile"), " ", (0,
                        t.jsxs)("span", {
                            className: "text-red-600 text-[10px]",
                            children: ["(", C("lblMustFace7"), ")"]
                        })]
                    }), (0,
                    t.jsx)("div", {
                        className: "mt-2",
                        children: (0,
                        t.jsx)("input", {
                            type: "text",
                            name: "webfile_id",
                            id: "webfile_id",
                            onCopy: e => e.preventDefault(),
                            onPaste: e => e.preventDefault(),
                            onCut: e => e.preventDefault(),
                            maxLength: 12,
                            autoComplete: "off",
                            value: M,
                            onChange: e => {
                                H(e.target.value),
                                G(!1)
                            }
                            ,
                            className: "block w-full rounded-md bg-white px-3 py-1.5 text-base text-black outline-1 -outline-offset-1 outline-gray-300 placeholder:text-gray-400 sm:text-sm/6"
                        })
                    })]
                }), (0,
                t.jsxs)("div", {
                    className: "sm:col-span-3",
                    children: [(0,
                    t.jsxs)("label", {
                        htmlFor: "first-name",
                        className: "block text-xs font-light text-black uppercase",
                        children: [C("lblInputWebFileAgain"), " ", (0,
                        t.jsxs)("span", {
                            className: "text-red-600 text-[10px]",
                            children: ["*(", C("lblNoCopyPaste"), ")"]
                        })]
                    }), (0,
                    t.jsx)("div", {
                        className: "mt-2",
                        children: (0,
                        t.jsx)("input", {
                            type: "text",
                            name: "first-name",
                            id: "first-name",
                            onCopy: e => e.preventDefault(),
                            onPaste: e => e.preventDefault(),
                            onCut: e => e.preventDefault(),
                            maxLength: 12,
                            autoComplete: "off",
                            value: B,
                            onChange: e => {
                                K(e.target.value),
                                G(!1)
                            }
                            ,
                            className: "block w-full rounded-md bg-white px-3 py-1.5 text-base text-black outline-1 outline-gray-300 placeholder:text-gray-400 focus:outline-1 focus:-outline-offset-1 sm:text-sm/6"
                        })
                    })]
                }), (0,
                t.jsxs)("div", {
                    className: "sm:col-span-3",
                    onClick: () => {
                        W || O({
                            message: "Please enter a valid Web File ID before selecting a center.",
                            type: "error"
                        })
                    }
                    ,
                    children: [(0,
                    t.jsx)("label", {
                        htmlFor: "first-name",
                        className: "block text-xs font-light text-black uppercase",
                        children: C("lblInputSelectIvac")
                    }), (0,
                    t.jsx)("div", {
                        className: "mt-2",
                        children: (0,
                        t.jsxs)("select", {
                            id: "center",
                            name: "center",
                            disabled: !W,
                            value: Q && X ? `${Q}|${X}` : "",
                            onChange: e => {
                                let[t,a] = e.target.value.split("|");
                                Z(t),
                                ee(a)
                            }
                            ,
                            className: "block w-full rounded-md px-3 py-1.5 text-base font-light text-black outline-1 -outline-offset-1 outline-gray-300 placeholder:text-gray-400  sm:text-sm/6",
                            children: [(0,
                            t.jsx)("option", {
                                value: "",
                                disabled: !0,
                                hidden: !0,
                                children: C("lblInputSelectIvac")
                            }), Y.map(e => (0,
                            t.jsx)("option", {
                                value: `${e.app_key}|${e.id}`,
                                children: e.ivac_name
                            }, e.id))]
                        })
                    })]
                }), (0,
                t.jsxs)("div", {
                    className: "sm:col-span-3",
                    children: [(0,
                    t.jsxs)("label", {
                        htmlFor: "first-name",
                        className: "block text-xs font-light text-black uppercase",
                        children: [C("lblInputVisaType"), " ", (0,
                        t.jsxs)("span", {
                            className: "text-red-600 text-[10px]",
                            children: ["*(", C("lblVisaTypeQueue"), ")"]
                        })]
                    }), (0,
                    t.jsx)("div", {
                        className: "mt-2",
                        children: (0,
                        t.jsxs)("select", {
                            id: "visa_type",
                            name: "visa_type",
                            disabled: !W,
                            value: el ?? "",
                            onChange: e => {
                                es(e.target.value)
                            }
                            ,
                            className: "block w-full rounded-md px-3 py-1.5 text-base font-light text-black outline-1 -outline-offset-1 outline-gray-300 placeholder:text-gray-400 sm:text-sm/6",
                            children: [(0,
                            t.jsx)("option", {
                                value: "",
                                disabled: !0,
                                hidden: !0,
                                children: C("lblInputVisaTypePlaceholder")
                            }), et.map(e => (0,
                            t.jsx)("option", {
                                value: e.id,
                                children: e.type_name
                            }, e.id))]
                        })
                    })]
                }), (0,
                t.jsxs)("div", {
                    className: "sm:col-span-3",
                    children: [(0,
                    t.jsx)("label", {
                        htmlFor: "first-name",
                        className: "block text-xs font-light text-black uppercase",
                        children: C("lblInputNumberOfFamily")
                    }), (0,
                    t.jsx)("div", {
                        className: "mt-2",
                        children: (0,
                        t.jsxs)("select", {
                            id: "family_count",
                            name: "family_count",
                            disabled: !W,
                            value: ei ?? "",
                            onChange: e => {
                                en(e.target.value)
                            }
                            ,
                            className: "block w-full rounded-md px-3 py-1.5 text-base font-light text-black outline-1 -outline-offset-1 outline-gray-300 placeholder:text-gray-400 sm:text-sm/6",
                            children: [(0,
                            t.jsx)("option", {
                                value: "",
                                disabled: !0,
                                hidden: !0,
                                children: C("lblInputNoOfAppPlaceholder")
                            }), (0,
                            t.jsx)("option", {
                                value: "0",
                                children: "0"
                            }), (0,
                            t.jsx)("option", {
                                value: "1",
                                children: "1"
                            }), (0,
                            t.jsx)("option", {
                                value: "2",
                                children: "2"
                            }), (0,
                            t.jsx)("option", {
                                value: "3",
                                children: "3"
                            })]
                        })
                    })]
                }), (0,
                t.jsxs)("div", {
                    className: "sm:col-span-3",
                    children: [(0,
                    t.jsxs)("label", {
                        htmlFor: "first-name",
                        className: "block text-xs font-light text-black uppercase",
                        children: [C("lblInputVisitPurpose"), " ", (0,
                        t.jsxs)("span", {
                            className: "text-red-600 uppercase text-[10px]",
                            children: ["*(", C("noCopyPasteMinChar"), ")"]
                        })]
                    }), (0,
                    t.jsx)("div", {
                        className: "mt-2",
                        children: (0,
                        t.jsx)("textarea", {
                            name: "visit_purpose",
                            id: "visit_purpose",
                            onCopy: e => e.preventDefault(),
                            onPaste: e => e.preventDefault(),
                            onCut: e => e.preventDefault(),
                            autoComplete: "off",
                            disabled: !W,
                            value: er,
                            onChange: e => {
                                eo(e.target.value)
                            }
                            ,
                            className: "block w-full rounded-md bg-white px-3 py-1.5 text-base text-black outline-1 -outline-offset-1 outline-gray-300 placeholder:text-gray-400 sm:text-sm/6"
                        })
                    })]
                }), (0,
                t.jsxs)("div", {
                    className: "sm:col-span-3",
                    children: [(0,
                    t.jsx)("label", {
                        htmlFor: "first-name",
                        className: "block text-xs font-light text-black uppercase",
                        children: C("lblInputAmount")
                    }), (0,
                    t.jsxs)("label", {
                        htmlFor: "first-name",
                        className: "block text-sm font-bold text-black uppercase",
                        children: ["BDT ", 0 !== m ? m : ""]
                    })]
                }), (0,
                t.jsx)("div", {
                    className: "sm:col-span-3",
                    children: (0,
                    t.jsx)(r.default, {
                        sitekey: "0x4AAAAAABvQ3Mi6RktCuZ7P",
                        onVerify: e => eu(e)
                    })
                }), (0,
                t.jsx)("div", {
                    className: "sm:col-span-3",
                    children: F ? (0,
                    t.jsxs)(a.motion.button, {
                        type: "button",
                        whileTap: {
                            scale: .95
                        },
                        className: `text-white mb-2 mt-2 h-8 rounded-lg text-xs w-full sm:w-auto px-2 py-2 text-center transition duration-300
              bg-gray-400 cursor-not-allowed
            `,
                        children: [C("checking"), "..."]
                    }) : (0,
                    t.jsx)(a.motion.button, {
                        type: "button",
                        disabled: !ex,
                        whileTap: {
                            scale: .95
                        },
                        className: `text-white mb-2 mt-2 h-8 rounded-lg text-xs w-full sm:w-auto px-2 py-2 text-center transition duration-300
              ${!ex ? "bg-gray-400 cursor-not-allowed" : "bg-[#28a745] hover:bg-[#218838] cursor-pointer"}
            `,
                        onClick: ex ? ef : void 0,
                        children: C("lblSaveNext")
                    })
                })]
            }) : (0,
            t.jsx)("div", {
                className: "sm:col-span-3 flex justify-center h-[50vh]",
                children: (0,
                t.jsx)("p", {
                    className: "flex items-center justify-center w-full h-screen-1/2  text-red-500 text-sm font-light rounded-md",
                    children: C("slotAvailableText") ?? "Please wait until slot is available."
                })
            })]
        })
    }
    ;
    var w = (0,
    u.createServerReference)("706597cd5e9803020d2c67c2d54323975b19fff992", u.callServer, void 0, u.findSourceMapURL, "submitPersonalInfo")
      , N = e.i(7910);
    let S = () => {
        let[e,r] = (0,
        i.useState)(!1)
          , c = (0,
        l.useTranslations)("root")
          , u = ((0,
        n.useRouter)(),
        (0,
        s.useDispatch)())
          , {user: p} = (0,
        s.useSelector)(e => e.authUser)
          , x = (0,
        s.useSelector)(e => e.application.isEdit)
          , f = (0,
        s.useSelector)(e => e.ip.clientIp)
          , {applicationData: h} = (0,
        s.useSelector)(e => e.application)
          , {personalInfo: b} = (0,
        s.useSelector)(e => e.personalInfo)
          , [y,v] = (0,
        i.useState)(null)
          , [g,_] = (0,
        i.useState)(!1)
          , [j,S] = (0,
        i.useState)("")
          , [k,A] = (0,
        i.useState)("")
          , [C,I] = (0,
        i.useState)("")
          , [T,D] = (0,
        i.useState)("")
          , [R,F] = (0,
        i.useState)(0)
          , [P,V] = (0,
        i.useState)({});
        (0,
        i.useEffect)( () => {
            p && (S(p.name),
            A(p.email),
            I(p.user_phone)),
            b && V(b.family),
            h && (D(h.webfile_id),
            F(parseInt(h.family_count, 10) || 0))
        }
        , [p, h, b]);
        let O = (e, t, a) => {
            let l = {
                ...P,
                [e + 1]: {
                    ...P[e + 1],
                    [t]: a
                }
            };
            V(l),
            u((0,
            N.setPersonalInfo)({
                family: l
            }))
        }
          , E = async () => {
            let {valid: e, errors: t} = ( () => {
                let e = [];
                if (j.trim() || e.push("Full name is required."),
                k.trim() || e.push("Email is required."),
                C.trim() || e.push("Phone is required."),
                T.trim() || e.push("Web file number is required."),
                R > 0)
                    for (let t = 1; t <= R; t++) {
                        let a = P[t];
                        a?.name?.trim() || e.push(`Family member ${t}: Name is required.`),
                        a?.webfile_no?.trim() || e.push(`Family member ${t}: Web file number is required.`),
                        a?.again_webfile_no?.trim() || e.push(`Family member ${t}: Confirm web file number is required.`)
                    }
                return e.length > 0 ? {
                    valid: !1,
                    errors: e
                } : {
                    valid: !0
                }
            }
            )();
            if (!e && t && t.length > 0)
                return void v({
                    message: "Please fix the following error:\n" + t[0],
                    type: "error"
                });
            try {
                _(!0);
                let e = {
                    full_name: j,
                    email_name: k,
                    phone: C,
                    webfile_id: T,
                    family: P
                }
                  , t = await w(e, x, f || void 0);
                if (t?.status_code === 200)
                    u((0,
                    N.setPersonalInfo)(e)),
                    u((0,
                    d.setApplicationStep)(3)),
                    v({
                        message: "Amount change notified successfully",
                        type: "success"
                    });
                else {
                    let e, a;
                    e = t.message || "Failed to notify",
                    a = t.status_code || 422,
                    401 === a || 419 === a ? (v({
                        message: e,
                        type: "error"
                    }),
                    setTimeout( () => {
                        (0,
                        m.handleLogout)()
                    }
                    , 5e3)) : v({
                        message: e,
                        type: "error"
                    })
                }
            } catch {
                v({
                    message: "An unexpected error occurred. Please try again later.",
                    type: "error"
                })
            } finally {
                _(!1)
            }
        }
        ;
        return (0,
        i.useEffect)( () => {
            r(!1);
            let e = setTimeout( () => {
                r(!0)
            }
            , 1e4 + 35e3 * R);
            return () => clearTimeout(e)
        }
        , [R]),
        (0,
        t.jsxs)("div", {
            className: "grid gap-x-6 gap-y-2",
            children: [y && (0,
            t.jsx)(o.default, {
                message: y.message,
                type: y.type,
                onClose: () => v(null)
            }), (0,
            t.jsxs)("div", {
                className: "sm:col-span-3",
                children: [(0,
                t.jsx)("label", {
                    htmlFor: "full-name",
                    className: "block text-xs font-light text-black uppercase",
                    children: c("lblInputName")
                }), (0,
                t.jsx)("div", {
                    className: "mt-2",
                    children: (0,
                    t.jsx)("input", {
                        type: "text",
                        name: "full-name",
                        id: "full-name",
                        value: j,
                        disabled: !0,
                        autoComplete: "off",
                        placeholder: c("lblInputRegName"),
                        className: "block w-full rounded-md bg-white px-3 py-1.5 text-base text-black outline-1 -outline-offset-1 outline-gray-300 placeholder:text-gray-400 focus:outline-1 focus:-outline-offset-2 focus:outline-indigo-600 sm:text-sm/6"
                    })
                })]
            }), (0,
            t.jsxs)("div", {
                className: "sm:col-span-3",
                children: [(0,
                t.jsx)("label", {
                    htmlFor: "email",
                    className: "block text-xs font-light text-black uppercase",
                    children: c("lblInputEmail")
                }), (0,
                t.jsx)("div", {
                    className: "mt-2",
                    children: (0,
                    t.jsx)("input", {
                        type: "text",
                        name: "email",
                        id: "email",
                        onCopy: e => e.preventDefault(),
                        onPaste: e => e.preventDefault(),
                        onCut: e => e.preventDefault(),
                        value: k,
                        disabled: !0,
                        autoComplete: "off",
                        className: "block w-full rounded-md bg-white px-3 py-1.5 text-base text-black outline-1 -outline-offset-1 outline-gray-300 placeholder:text-gray-400 focus:outline-1 focus:-outline-offset-2 focus:outline-indigo-600 sm:text-sm/6"
                    })
                })]
            }), (0,
            t.jsxs)("div", {
                className: "sm:col-span-3",
                children: [(0,
                t.jsx)("label", {
                    htmlFor: "user_phone",
                    className: "block text-xs font-light text-black uppercase",
                    children: c("lblInputContactNo")
                }), (0,
                t.jsx)("div", {
                    className: "mt-2",
                    children: (0,
                    t.jsx)("input", {
                        type: "text",
                        name: "user_phone",
                        id: "user_phone",
                        onCopy: e => e.preventDefault(),
                        onPaste: e => e.preventDefault(),
                        onCut: e => e.preventDefault(),
                        value: C,
                        disabled: !0,
                        autoComplete: "off",
                        className: "block w-full rounded-md bg-white px-3 py-1.5 text-base text-black outline-1 -outline-offset-1 outline-gray-300 placeholder:text-gray-400 focus:outline-1 focus:-outline-offset-2 focus:outline-indigo-600 sm:text-sm/6"
                    })
                })]
            }), (0,
            t.jsxs)("div", {
                className: "sm:col-span-3",
                children: [(0,
                t.jsx)("label", {
                    htmlFor: "webfile_id",
                    className: "block text-xs font-light text-black uppercase",
                    children: c("ovWebID")
                }), (0,
                t.jsx)("div", {
                    className: "mt-2",
                    children: (0,
                    t.jsx)("input", {
                        type: "text",
                        name: "webfile_id",
                        id: "webfile_id",
                        value: T,
                        disabled: !0,
                        autoComplete: "off",
                        className: "block w-full rounded-md bg-white px-3 py-1.5 text-base text-black outline-1 -outline-offset-1 outline-gray-300 placeholder:text-gray-400 focus:outline-1 focus:-outline-offset-2 focus:outline-indigo-600 sm:text-sm/6"
                    })
                })]
            }), (0,
            t.jsxs)("div", {
                className: "sm:col-span-3",
                children: [[...Array(R)].map( (e, a) => (0,
                t.jsxs)("div", {
                    className: "flex flex-col w-full gap-2",
                    children: [(0,
                    t.jsxs)("div", {
                        className: "w-full",
                        children: [(0,
                        t.jsxs)("label", {
                            htmlFor: `full-name-${a}`,
                            className: "block text-xs font-light text-black uppercase my-2",
                            children: [c("lblInputFamily"), " ", a + 1]
                        }), (0,
                        t.jsxs)("label", {
                            htmlFor: `full-name-${a}`,
                            className: "block text-xs font-light text-black uppercase",
                            children: [c("lblInputFullName"), " ", (0,
                            t.jsxs)("span", {
                                className: "text-red-600 text-[10px]",
                                children: ["*(", c("lblRequired"), ")"]
                            })]
                        }), (0,
                        t.jsx)("div", {
                            className: "mt-2",
                            children: (0,
                            t.jsx)("input", {
                                type: "text",
                                name: `full-name-${a}`,
                                id: `full-name-${a}`,
                                onCopy: e => e.preventDefault(),
                                onPaste: e => e.preventDefault(),
                                onCut: e => e.preventDefault(),
                                value: P[a + 1]?.name || "",
                                onChange: e => O(a, "name", e.target.value),
                                autoComplete: "off",
                                className: "block w-full rounded-md bg-white px-3 py-1.5 text-base text-black outline-1 -outline-offset-1 outline-gray-300 placeholder:text-gray-400 sm:text-sm/6"
                            })
                        })]
                    }), (0,
                    t.jsxs)("div", {
                        className: "sm:col-span-3",
                        children: [(0,
                        t.jsxs)("label", {
                            htmlFor: `web-file-number-${a}`,
                            className: "block text-xs font-light text-black uppercase",
                            children: [c("lblInputFamilyWebFile"), " ", (0,
                            t.jsxs)("span", {
                                className: "text-red-600 text-[10px]",
                                children: ["*(", c("lblMustFace7"), ")"]
                            })]
                        }), (0,
                        t.jsx)("div", {
                            className: "mt-2",
                            children: (0,
                            t.jsx)("input", {
                                type: "text",
                                name: `web-file-number-${a}`,
                                id: `web-file-number-${a}`,
                                onCopy: e => e.preventDefault(),
                                onPaste: e => e.preventDefault(),
                                onCut: e => e.preventDefault(),
                                value: P[a + 1]?.webfile_no || "",
                                onChange: e => O(a, "webfile_no", e.target.value),
                                autoComplete: "off",
                                className: "block w-full rounded-md bg-white px-3 py-1.5 text-base text-black outline-1 -outline-offset-1 outline-gray-300 placeholder:text-gray-400 sm:text-sm/6"
                            })
                        })]
                    }), (0,
                    t.jsxs)("div", {
                        className: "sm:col-span-3",
                        children: [(0,
                        t.jsxs)("label", {
                            htmlFor: `web-file-number-repeat-${a}`,
                            className: "block text-xs font-light text-black uppercase",
                            children: [c("lblInputFamilyWebFileAgain"), " ", (0,
                            t.jsxs)("span", {
                                className: "text-red-600 text-[10px]",
                                children: ["*(", c("lblNoCopyPaste"), ")"]
                            })]
                        }), (0,
                        t.jsx)("div", {
                            className: "mt-2",
                            children: (0,
                            t.jsx)("input", {
                                type: "text",
                                name: `web-file-number-repeat-${a}`,
                                id: `web-file-number-repeat-${a}`,
                                onCopy: e => e.preventDefault(),
                                onPaste: e => e.preventDefault(),
                                onCut: e => e.preventDefault(),
                                value: P[a + 1]?.again_webfile_no || "",
                                onChange: e => O(a, "again_webfile_no", e.target.value),
                                autoComplete: "off",
                                className: "block w-full rounded-md bg-white px-3 py-1.5 text-base text-black outline-1 -outline-offset-1 outline-gray-300 placeholder:text-gray-400 sm:text-sm/6"
                            })
                        })]
                    })]
                }, a)), (0,
                t.jsx)("div", {
                    className: "sm:col-span-3",
                    children: g ? (0,
                    t.jsxs)(a.motion.button, {
                        type: "button",
                        whileTap: {
                            scale: .95
                        },
                        className: "text-white mb-2 duration-300 cursor-not-allowed mt-2 h-8 rounded-lg text-xs w-full sm:w-auto px-2 py-2 text-center transition bg-gray-500 ",
                        children: [c("checking"), "..."]
                    }) : (0,
                    t.jsx)(a.motion.button, {
                        type: "button",
                        whileTap: {
                            scale: .95
                        },
                        className: `text-white mb-2 duration-300 mt-2 h-8 rounded-lg text-xs w-full sm:w-auto px-2 py-2 text-center transition bg-[#28a745] hover:bg-[#218838] ${e ? "cursor-pointer" : "cursor-not-allowed opacity-50"}`,
                        onClick: e ? E : void 0,
                        disabled: !e,
                        children: c("lblSaveNext")
                    })
                })]
            })]
        })
    }
    ;
    var k = e.i(63762)
      , A = (0,
    u.createServerReference)("609be894c11698a9fdc88b9c6f9fa821916c7fb66d", u.callServer, void 0, u.findSourceMapURL, "submitOverview")
      , C = e.i(61745);
    let I = () => {
        let e = (0,
        l.useTranslations)("root")
          , r = ((0,
        n.useRouter)(),
        (0,
        s.useDispatch)())
          , c = (0,
        C.useLocale)()
          , {applicationData: u} = (0,
        s.useSelector)(e => e.application)
          , {personalInfo: p} = (0,
        s.useSelector)(e => e.personalInfo)
          , x = (0,
        s.useSelector)(e => e.authUser)
          , f = (0,
        s.useSelector)(e => e.ip.clientIp)
          , [b,y] = (0,
        i.useState)(!1)
          , [_,j] = (0,
        i.useState)(0)
          , [w,N] = (0,
        i.useState)(-1)
          , [S,I] = (0,
        i.useState)(null)
          , [T,D] = (0,
        i.useState)(null)
          , [R,F] = (0,
        i.useState)(null)
          , [P,V] = (0,
        i.useState)(null)
          , [O,E] = (0,
        i.useState)(null)
          , [L,U] = (0,
        i.useState)(null)
          , [$,M] = (0,
        i.useState)({})
          , [H,B] = (0,
        i.useState)(!1)
          , [K,W] = (0,
        i.useState)(null)
          , [G,q] = (0,
        i.useState)(null)
          , [J,Y] = (0,
        i.useState)(!1)
          , z = async () => {
            if (J)
                try {
                    B(!0);
                    let e = await A(c, f || void 0);
                    if (e?.status_code === 200)
                        localStorage.removeItem("applicant"),
                        localStorage.removeItem("personal_info"),
                        r((0,
                        d.setApplicationStep)(4)),
                        I({
                            message: e.data?.message || "Overview submitted successfully.",
                            type: "success"
                        });
                    else {
                        let t, a;
                        t = e.message || "Failed to submit overview.",
                        a = e.status_code || 422,
                        401 === a || 419 === a ? (I({
                            message: t,
                            type: "error"
                        }),
                        setTimeout( () => {
                            (0,
                            m.handleLogout)()
                        }
                        , 5e3)) : I({
                            message: t,
                            type: "error"
                        })
                    }
                } catch (e) {
                    I({
                        message: "An unexpected error occurred. Please try again later.",
                        type: "error"
                    })
                } finally {
                    B(!1)
                }
        }
        ;
        (0,
        i.useEffect)( () => {}
        , []),
        (0,
        i.useEffect)( () => {
            if (x?.user && (D(x.user.name || ""),
            F(x.user.email || ""),
            E(x.user.user_phone || "")),
            p && M(p.family),
            u) {
                if (V(x.visa_fee || 1500),
                U(u.webfile_id || null),
                void 0 !== u.ivac_id) {
                    let e = v.find(e => e.id == Number(u.ivac_id));
                    e && W(e.ivac_name)
                }
                if (void 0 !== u.visa_type) {
                    let e = g.find(e => e.id === Number(u.visa_type));
                    e && q(e.type_name)
                }
            }
        }
        , [u, x, p]),
        (0,
        i.useEffect)( () => {
            u?.family_count && j(parseInt(u.family_count, 10))
        }
        , [u]),
        (0,
        i.useEffect)( () => {
            let e = setTimeout( () => {
                y(!0)
            }
            , 1e4);
            return () => clearTimeout(e)
        }
        , []);
        let Q = e => {
            N(t => t === e ? null : e)
        }
          , Z = {
            hidden: {
                opacity: 0,
                y: 10,
                scale: .95
            },
            visible: {
                opacity: 1,
                y: 0,
                scale: 1,
                transition: {
                    duration: .3
                }
            }
        }
          , X = ["ovWebID", "ovIvacCenter", "ovVisaType", "ovFees", "ovName", "ovEmail", "ovContact"];
        return (0,
        t.jsxs)(a.motion.div, {
            className: "flex flex-col",
            variants: {
                hidden: {
                    opacity: 0,
                    y: 40
                },
                visible: {
                    opacity: 1,
                    y: 0,
                    transition: {
                        duration: .6
                    }
                }
            },
            initial: "hidden",
            animate: "visible",
            children: [S && (0,
            t.jsx)(o.default, {
                message: S.message,
                type: S.type,
                onClose: () => I(null)
            }), (0,
            t.jsxs)("div", {
                className: "flex flex-row gap-2 items-center p-2 bg-green-50 border-[0.2px] border-green-100 rounded cursor-pointer",
                onClick: () => Q(-1),
                children: [(0,
                t.jsxs)("p", {
                    className: "text-xs md:text-sm font-light text-white rounded-md bg-black text-center p-2 min-w-[110px]",
                    children: [e("application_text"), " #1"]
                }), (0,
                t.jsxs)("p", {
                    className: "text-green-700 text-xs hover:cursor-pointer font-bold cursor-pointer hover:underline",
                    children: [L ?? "", " (", e("btnExpandOrClose"), ")"]
                })]
            }), (0,
            t.jsx)(a.motion.div, {
                initial: !1,
                animate: {
                    height: -1 === w ? "auto" : 0,
                    opacity: +(-1 === w)
                },
                transition: {
                    duration: .3
                },
                className: `overflow-hidden mt-2 rounded-md border-[0.2px] border-slate-300 ${-1 === w ? "py-4 mb-4" : ""}`,
                children: -1 === w && (0,
                t.jsxs)(a.motion.div, {
                    className: "flex flex-wrap md:px-4 relative text-center justify-center",
                    children: [X.map( (l, s) => (0,
                    t.jsxs)(a.motion.div, {
                        variants: Z,
                        initial: "hidden",
                        animate: "visible",
                        className: "flex flex-col flex-wrap items-center justify-center m-2 bg-gray-200 min-h-12 text-sm font-light shadow-md rounded-md px-2 py-1 text-center w-[180px] md:w-1/3",
                        children: [(0,
                        t.jsx)("p", {
                            className: "text-red-600 font-light text-[8px] md:text-[10px] break-words text-center w-full",
                            children: e(l)
                        }), (0,
                        t.jsx)("p", {
                            className: "text-[12px] font-semibold break-words text-center w-full",
                            children: "ovWebID" === l ? L || "N/A" : "ovIvacCenter" === l ? K || "N/A" : "ovVisaType" === l ? G || "N/A" : "ovEmail" === l ? R || "N/A" : "ovName" === l ? T || "N/A" : "ovContact" === l ? O || "N/A" : "ovFees" === l ? "BDT " + P : "NA"
                        })]
                    }, s)), (0,
                    t.jsx)("div", {
                        className: "col-span-3 z-20 -mt-20 relative right-0 w-full flex justify-end items-end",
                        children: (0,
                        t.jsxs)("div", {
                            className: "flex items-center gap-1 hover:text-green-800 duration-300 font-bold text-xs cursor-pointer w-fit pr-4 md:pr-0",
                            onClick: () => {
                                r((0,
                                d.setApplicationStep)(1)),
                                r((0,
                                h.setIsEdit)(!0))
                            }
                            ,
                            children: [e("edit_text"), " ", (0,
                            t.jsx)(k.EditIcon, {})]
                        })
                    })]
                })
            }), [...Array(_)].map( (l, s) => (0,
            t.jsxs)(a.motion.div, {
                initial: "hidden",
                animate: "visible",
                children: [(0,
                t.jsxs)("div", {
                    className: "flex flex-row gap-2 items-center p-2 bg-green-50 border-[0.2px] border-green-100 rounded cursor-pointer",
                    onClick: () => Q(s),
                    children: [(0,
                    t.jsxs)("p", {
                        className: "text-xs md:text-sm font-light text-white rounded-md bg-black text-center p-2 min-w-[110px]",
                        children: [e("application_text"), " #", s + 2]
                    }), (0,
                    t.jsxs)("p", {
                        className: "text-green-700 text-xs hover:cursor-pointer font-bold cursor-pointer hover:underline",
                        children: [Object.values($)[s]?.webfile_no, " (", e("btnExpandOrClose"), ")"]
                    })]
                }), (0,
                t.jsx)(a.motion.div, {
                    initial: !1,
                    animate: {
                        height: w === s ? "auto" : 0,
                        opacity: +(w === s)
                    },
                    transition: {
                        duration: .3
                    },
                    className: `overflow-hidden mt-2 rounded-md border-[0.2px] border-slate-300 ${w === s ? "py-4 mb-4" : ""}`,
                    children: w === s && (0,
                    t.jsxs)(a.motion.div, {
                        className: "flex flex-wrap md:px-4 relative text-center justify-center",
                        children: [X.map( (l, i) => (0,
                        t.jsxs)(a.motion.div, {
                            variants: Z,
                            initial: "hidden",
                            animate: "visible",
                            className: "flex flex-col flex-wrap items-center justify-center m-2 bg-gray-200 min-h-12 text-sm font-light shadow-md rounded-md px-2 py-1 text-center w-[180px] md:w-1/3",
                            children: [(0,
                            t.jsx)("p", {
                                className: "text-red-600 font-light text-[8px] md:text-[10px] break-words text-center w-full",
                                children: e(l)
                            }), (0,
                            t.jsx)("p", {
                                className: "text-[12px] font-semibold break-words text-center w-full",
                                children: "ovWebID" === l ? Object.values($)[s]?.webfile_no || "N/A" : "ovIvacCenter" === l ? K || "N/A" : "ovVisaType" === l ? G || "N/A" : "ovEmail" === l ? R || "N/A" : "ovName" === l ? Object.values($)[s]?.name || "N/A" : "ovContact" === l ? O || "N/A" : "ovFees" === l ? "BDT " + P : "NA"
                            })]
                        }, i)), (0,
                        t.jsx)("div", {
                            className: "col-span-3 z-20 -mt-20 relative right-0 w-full flex justify-end items-end",
                            children: (0,
                            t.jsxs)("div", {
                                className: "flex items-center gap-1 hover:text-green-900 duration-300 font-bold text-xs cursor-pointer w-fit pr-4 md:pr-0",
                                onClick: () => {
                                    r((0,
                                    d.setApplicationStep)(2)),
                                    r((0,
                                    h.setIsEdit)(!0))
                                }
                                ,
                                children: [e("edit_text"), " ", (0,
                                t.jsx)(k.EditIcon, {})]
                            })
                        })]
                    })
                })]
            }, s)), (0,
            t.jsxs)("div", {
                className: "space-y-2 max-w-md px-2 mt-4",
                children: [(0,
                t.jsxs)("div", {
                    className: "flex items-center space-x-2",
                    children: [(0,
                    t.jsx)("input", {
                        type: "checkbox",
                        id: "terms",
                        checked: J,
                        onChange: e => Y(e.target.checked)
                    }), (0,
                    t.jsxs)("label", {
                        htmlFor: "terms",
                        className: "text-sm font-light leading-none peer-disabled:cursor-not-allowed peer-disabled:opacity-70",
                        children: [e("tcAgree"), " ", (0,
                        t.jsx)("a", {
                            href: "https://api-payment.ivacbd.com/contents/tos.html",
                            target: "_blank",
                            className: "text-blue-600 hover:underline",
                            children: e("tcText")
                        })]
                    })]
                }), (0,
                t.jsx)("button", {
                    onClick: J && b ? z : void 0,
                    disabled: !J || !b || H,
                    className: `text-white mb-2 mt-2 h-8 rounded-lg text-xs w-full sm:w-auto px-2 py-2 text-center transition 
                      ${J && b && !H ? "bg-[#28a745] hover:bg-[#218838] cursor-pointer" : "bg-gray-400 cursor-not-allowed"}
                    `,
                    children: e("conMovePay")
                })]
            })]
        })
    }
    ;
    e.i(12507).default,
    e.i(74313).default,
    e.i(62778).default,
    e.i(56061).default,
    e.i(68727).default;
    let T = {
        src: e.i(10053).default,
        width: 44,
        height: 9,
        blurWidth: 0,
        blurHeight: 0
    };
    e.i(59027).default,
    e.i(25861).default,
    e.i(38875).default,
    e.i(39982).default,
    e.i(93559).default,
    e.i(88773).default,
    e.i(67879).default,
    e.i(69131).default,
    e.i(30801).default,
    e.i(8573).default,
    e.i(6262).default,
    e.i(1662).default;
    var D = e.i(16715)
      , R = (0,
    u.createServerReference)("40eca920744adac77a354fc0c9d8ea6319825135be", u.callServer, void 0, u.findSourceMapURL, "fetchPaymentData")
      , F = (0,
    u.createServerReference)("60f17fb803892fb0b32edd6a8970820aaeccaea2a1", u.callServer, void 0, u.findSourceMapURL, "sendOtpRequest")
      , P = (0,
    u.createServerReference)("607dd046ce4104a30dbcdae5ed02bdae7160e39745", u.callServer, void 0, u.findSourceMapURL, "verifyOtp")
      , V = (0,
    u.createServerReference)("60fd1e878d947ecff768e3fa5d1049d5bf42b6f548", u.callServer, void 0, u.findSourceMapURL, "handleAppointmentDateChange")
      , O = (0,
    u.createServerReference)("70648dcd61dd835363065814f031d26034cc029dfa", u.callServer, void 0, u.findSourceMapURL, "verifyCaptchaData")
      , E = (0,
    u.createServerReference)("60e3bf50cd36841322301d5ae4f1f1df55d0e26cd2", u.callServer, void 0, u.findSourceMapURL, "submitPayment");
    let L = (e, t, a, l) => {
        let s = e.message || t
          , i = e.status_code || 422;
        401 === i || 419 === i ? (l({
            message: s,
            type: "error"
        }),
        setTimeout( () => {
            (0,
            m.handleLogout)()
        }
        , 5e3)) : l({
            message: s,
            type: "error"
        })
    }
      , U = () => {
        let e = (0,
        n.useRouter)()
          , r = (0,
        l.useTranslations)("root")
          , d = (0,
        s.useSelector)(e => e.ip.clientIp)
          , [m,u] = (0,
        i.useState)(null)
          , [p,x] = (0,
        i.useState)(!1)
          , [f,h] = (0,
        i.useState)(null)
          , [b,y] = (0,
        i.useState)(0)
          , [v,g] = (0,
        i.useState)(0)
          , [_,j] = (0,
        i.useState)(0)
          , [w,N] = (0,
        i.useState)("")
          , [S,k] = (0,
        i.useState)(!1)
          , [A,C] = (0,
        i.useState)("")
          , [I,U] = (0,
        i.useState)(!1)
          , [$,M] = (0,
        i.useState)([])
          , [H,B] = (0,
        i.useState)("")
          , [K,W] = (0,
        i.useState)(!1)
          , [G,q] = (0,
        i.useState)([])
          , [J,Y] = (0,
        i.useState)("")
          , [z,Q] = (0,
        i.useState)("")
          , [Z,X] = (0,
        i.useState)("")
          , [ee,et] = (0,
        i.useState)("")
          , [ea,el] = (0,
        i.useState)(!1)
          , [es,ei] = (0,
        i.useState)(!1)
          , [en,er] = (0,
        i.useState)("")
          , [eo,ec] = (0,
        i.useState)("")
          , [ed,em] = (0,
        i.useState)(!1)
          , [eu,ep] = (0,
        i.useState)(!1)
          , [ex,ef] = (0,
        i.useState)(!1)
          , [eh,eb] = (0,
        i.useState)(!1)
          , [ey,ev] = (0,
        i.useState)(0)
          , eg = async () => {
            try {
                ef(!0);
                let t = await O(void 0, void 0, d || void 0);
                t?.status_code === 200 ? (Q(t.data.captcha_image),
                X(t.data.captcha_id),
                et(""),
                el(!1),
                er("")) : L(t, "Failed to get captcha. Please try again later.", e, u)
            } catch (e) {
                u({
                    message: "An unexpected error occurred. Please try again later.",
                    type: "error"
                })
            } finally {
                ef(!1)
            }
        }
          , e_ = async () => {
            try {
                ef(!0),
                ei(!0);
                let e = await O(Z, ee, d || void 0);
                e?.status_code === 200 ? (el(!0),
                er("")) : er(e.message || "Failed to verify captcha. Please try again.")
            } catch (e) {
                u({
                    message: "An unexpected error occurred. Please try again later.",
                    type: "error"
                })
            } finally {
                ef(!1),
                ei(!1)
            }
        }
        ;
        (0,
        i.useEffect)( () => {
            (async () => {
                try {
                    let t = await R(d || void 0);
                    t?.status_code === 200 ? (g(t.data?.convenience_fees),
                    y(t.data?.fees),
                    j(t.data?.payable_amount),
                    N(t.data?.mobile_no),
                    h(t.data?.payment_options?.data)) : L(t, "Failed to get payment data. Please try again later.", e, u)
                } catch (e) {
                    u({
                        message: "An unexpected error occurred. Please try again later.",
                        type: "error"
                    })
                }
            }
            )()
        }
        , [e]);
        let ej = async t => {
            try {
                em(!0);
                let a = await F(t, d || void 0);
                if (a?.status_code === 200) {
                    k(!0),
                    ev(30);
                    let e = setInterval( () => {
                        ev(t => t <= 1 ? (clearInterval(e),
                        0) : t - 1)
                    }
                    , 1e3)
                } else
                    L(a, "Failed to send OTP. Please try again later.", e, u)
            } catch (e) {
                u({
                    message: "An unexpected error occurred. Please try again later.",
                    type: "error"
                })
            } finally {
                em(!1)
            }
        }
          , ew = async () => {
            try {
                if (A.length < 6)
                    return void u({
                        message: "Please enter 6 digit otp",
                        type: "error"
                    });
                ep(!0);
                let t = await P(A, d || void 0);
                t?.status_code === 200 ? (U(!0),
                M(t.data?.slot_dates)) : L(t, "Failed to verify OTP. Please try again later.", e, u)
            } catch (e) {
                u({
                    message: "An unexpected error occurred. Please try again later.",
                    type: "error"
                })
            } finally {
                ep(!1)
            }
        }
          , eN = async t => {
            let a = t.target.value;
            B(a);
            try {
                ef(!0);
                let t = await V(a, d || void 0);
                t?.status_code === 200 ? (q(t.data?.slot_times ?? []),
                W(!0),
                Q(t.data?.captcha?.captcha_image),
                X(t.data?.captcha?.captcha_id),
                et(""),
                el(!1),
                er("")) : L(t, "Failed to fetch appointment times. Please try again later.", e, u)
            } catch (e) {
                u({
                    message: "An unexpected error occurred. Please try again later.",
                    type: "error"
                })
            } finally {
                ef(!1)
            }
        }
          , [eS,ek] = (0,
        i.useState)(null)
          , eA = "" !== H.trim() && H.length >= 6 && "" !== J.trim() && J.length >= 6 && null !== eS && "" !== eS.item.name.trim() && "" !== eS.item.slug.trim() && "" !== eS.item.link.trim() && "" !== ee.trim() && ee.length >= 6 && ea
          , eC = async () => {
            if (!eS)
                return void u({
                    message: "Please select a payment option.",
                    type: "error"
                });
            try {
                eb(!0);
                let t = new FormData;
                t.append("appointment_date", H),
                t.append("appointment_time", J),
                t.append("k5t0g8_token_y4v9f6", Z),
                t.append("selected_payment[name]", eS.item.name),
                t.append("selected_payment[slug]", eS.item.slug),
                t.append("selected_payment[link]", eS.item.link);
                let a = await E(t, d || void 0);
                a?.status_code === 200 ? (document.cookie.split(";").forEach(e => {
                    document.cookie = e.replace(/^ +/, "").replace(/=.*/, "=;expires=" + new Date().toUTCString() + ";path=/")
                }
                ),
                localStorage.clear(),
                window.location.href = a.data.url) : L(a, "Failed to process payment. Please try again later.", e, u)
            } catch (e) {
                u({
                    message: "An unexpected error occurred. Please try again later.",
                    type: "error"
                })
            } finally {
                eb(!1)
            }
        }
        ;
        return ((0,
        i.useEffect)( () => {
            let e = setTimeout( () => {
                x(!0)
            }
            , 1e4);
            return () => clearTimeout(e)
        }
        , []),
        ex) ? (0,
        t.jsx)(c, {}) : (0,
        t.jsxs)(a.motion.div, {
            className: "flex flex-col",
            variants: {
                hidden: {
                    opacity: 0,
                    y: 40
                },
                visible: {
                    opacity: 1,
                    y: 0,
                    transition: {
                        duration: .6
                    }
                }
            },
            initial: "hidden",
            animate: "visible",
            children: [m && (0,
            t.jsx)(o.default, {
                message: m.message,
                type: m.type,
                onClose: () => u(null)
            }), (0,
            t.jsx)("div", {
                className: "max-h-screen bg-gray-50 md:p-4",
                children: (0,
                t.jsxs)("div", {
                    className: "max-w-6xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-8",
                    children: [(0,
                    t.jsx)("div", {
                        className: "bg-white rounded-lg p-5 md:p-6 shadow-sm",
                        children: (0,
                        t.jsxs)("div", {
                            className: "space-y-6 h-full flex flex-col justify-around",
                            children: [(0,
                            t.jsxs)("div", {
                                className: "flex space-x-2 flex-col",
                                children: [(0,
                                t.jsxs)("h3", {
                                    className: "text-sm  text-gray-700 mb-3 font-bold",
                                    children: [r("payCard"), " "]
                                }), (0,
                                t.jsx)("div", {
                                    className: "boxes flex flex-row justify-start gap-4 flex-wrap",
                                    children: f?.cards && Object.keys(f.cards).length > 0 && Object.entries(f.cards).map( ([e,a]) => (0,
                                    t.jsx)("div", {
                                        className: `w-18 h-16 flex items-center justify-center rounded-lg border-[1px] border-slate-300 shadow hover:cursor-pointer hover:border-[#0074D4] duration-300 ${eS?.type === "cards" && eS.key === e ? "ring-2 ring-[#0074D4]" : ""}`,
                                        onClick: () => ek({
                                            type: "cards",
                                            key: e,
                                            item: a
                                        }),
                                        children: (0,
                                        t.jsx)("img", {
                                            src: a.link,
                                            alt: a.name,
                                            className: "w-10 h-10 object-contain"
                                        })
                                    }, e))
                                })]
                            }), (0,
                            t.jsxs)("div", {
                                className: "flex space-x-2 flex-col",
                                children: [(0,
                                t.jsxs)("h3", {
                                    className: "text-sm font-bold text-gray-700 mb-3",
                                    children: [r("payInt"), " "]
                                }), (0,
                                t.jsx)("div", {
                                    className: "boxes flex flex-row justify-start gap-4 flex-wrap",
                                    children: f?.internet && Object.keys(f.internet).length > 0 && Object.entries(f.internet).map( ([e,a]) => (0,
                                    t.jsx)("div", {
                                        className: `w-18 h-16 flex items-center justify-center rounded-lg border-[1px] border-slate-300 shadow hover:cursor-pointer hover:border-[#0074D4] duration-300  ${eS?.type === "internet" && eS.key === e ? "ring-2 ring-[#0074D4]" : ""}`,
                                        onClick: () => ek({
                                            type: "internet",
                                            key: e,
                                            item: a
                                        }),
                                        children: (0,
                                        t.jsx)("img", {
                                            src: a.link,
                                            alt: a.name,
                                            className: "w-full h-full object-contain"
                                        })
                                    }, e))
                                })]
                            }), (0,
                            t.jsxs)("div", {
                                className: "flex space-x-2 flex-col",
                                children: [(0,
                                t.jsxs)("h3", {
                                    className: "text-sm font-bold text-gray-700 mb-3",
                                    children: [r("payMob"), " "]
                                }), (0,
                                t.jsx)("div", {
                                    className: "boxes flex flex-row justify-start gap-4 flex-wrap",
                                    children: f?.mobile && Object.keys(f.mobile).length > 0 && Object.entries(f.mobile).map( ([e,a]) => (0,
                                    t.jsx)("div", {
                                        className: `w-18 h-16 flex items-center justify-center rounded-lg border-[1px] border-slate-300 shadow hover:cursor-pointer hover:border-[#0074D4] duration-300 ${eS?.type === "mobile" && eS.key === e ? "ring-2 ring-[#0074D4]" : ""}`,
                                        onClick: () => ek({
                                            type: "mobile",
                                            key: e,
                                            item: a
                                        }),
                                        children: (0,
                                        t.jsx)("img", {
                                            src: a.link,
                                            alt: a.name,
                                            className: "w-full h-full object-contain"
                                        })
                                    }, e))
                                })]
                            }), (0,
                            t.jsxs)("div", {
                                className: "flex space-x-2 flex-col",
                                children: [(0,
                                t.jsx)("h3", {
                                    className: "text-sm font-bold text-gray-700 mb-3",
                                    children: r("payOth")
                                }), (0,
                                t.jsx)("div", {
                                    className: "boxes flex flex-row justify-start gap-4 flex-wrap",
                                    children: f?.others && Object.keys(f.others).length > 0 && Object.entries(f.others).map( ([e,a]) => (0,
                                    t.jsx)("div", {
                                        className: `w-18 h-16 flex items-center justify-center rounded-lg border-[1px] border-slate-300 shadow hover:cursor-pointer hover:border-[#0074D4] duration-300 ${eS?.type === "others" && eS.key === e ? "ring-2 ring-[#0074D4]" : ""}`,
                                        onClick: () => ek({
                                            type: "others",
                                            key: e,
                                            item: a
                                        }),
                                        children: (0,
                                        t.jsx)("img", {
                                            src: a.link,
                                            alt: a.name,
                                            className: "w-full h-full object-contain"
                                        })
                                    }, e))
                                })]
                            })]
                        })
                    }), (0,
                    t.jsxs)("div", {
                        className: "bg-white rounded-lg p-6 shadow-sm",
                        children: [(0,
                        t.jsx)("div", {
                            className: "flex justify-end mb-4",
                            children: (0,
                            t.jsx)("div", {
                                className: " px-4 py-2 rounded-lg text-sm font-medium",
                                children: (0,
                                t.jsx)("img", {
                                    src: T.src,
                                    className: "w-36",
                                    alt: ""
                                })
                            })
                        }), (0,
                        t.jsx)("div", {
                            className: "bg-blue-50 border border-gray-200 rounded-lg px-4 py-2 mb-4",
                            children: eS ? (0,
                            t.jsxs)("div", {
                                className: "flex justify-between items-center ",
                                children: [(0,
                                t.jsx)("p", {
                                    className: "font-bold",
                                    children: eS?.item?.name
                                }), (0,
                                t.jsx)("img", {
                                    src: eS?.item?.link,
                                    alt: eS?.item?.name,
                                    className: "w-10 h-10 object-contain"
                                })]
                            }) : (0,
                            t.jsxs)("p", {
                                className: "text-gray-600 text-sm text-center",
                                children: [" ", r("paySelectOption"), " "]
                            })
                        }), (0,
                        t.jsxs)("div", {
                            className: "space-y-3 mb-4",
                            children: [(0,
                            t.jsxs)("div", {
                                className: "flex justify-between items-center",
                                children: [(0,
                                t.jsxs)("span", {
                                    className: "text-sm font-medium text-gray-700",
                                    children: [" ", r("ovFees"), " :"]
                                }), (0,
                                t.jsxs)("span", {
                                    className: "text-sm text-gray-900",
                                    children: [b, " BDT"]
                                })]
                            }), 0 != v && (0,
                            t.jsxs)("div", {
                                className: "flex justify-between items-center",
                                children: [(0,
                                t.jsxs)("span", {
                                    className: "text-sm text-gray-600",
                                    children: [" ", r("payConvFee"), " :"]
                                }), (0,
                                t.jsxs)("span", {
                                    className: "text-sm text-gray-900",
                                    children: [v, " BDT"]
                                })]
                            }), (0,
                            t.jsxs)("div", {
                                className: "flex justify-between items-center pt-2 border-t",
                                children: [(0,
                                t.jsxs)("span", {
                                    className: "text-sm font-medium text-gray-700",
                                    children: [r("payPayable"), ":"]
                                }), (0,
                                t.jsxs)("span", {
                                    className: "text-sm font-medium text-gray-900",
                                    children: [_, " BDT"]
                                })]
                            })]
                        }), (0,
                        t.jsxs)("div", {
                            className: "mb-2",
                            children: [(0,
                            t.jsxs)("label", {
                                className: "block text-sm font-medium text-gray-700 mb-2",
                                children: [r("payPhone"), ":"]
                            }), (0,
                            t.jsxs)("div", {
                                className: "flex items-center gap-3 rounded-md flex-wrap",
                                children: [(0,
                                t.jsx)("input", {
                                    type: "tel",
                                    value: w,
                                    disabled: !0,
                                    className: "flex-1 px-3 py-2 text-sm rounded-md border border-gray-300 bg-gray-100 text-gray-700 focus:outline-none",
                                    placeholder: r("payPhone"),
                                    autoComplete: "off"
                                }), !I && (0,
                                t.jsx)("button", {
                                    onClick: ed || ey > 0 ? void 0 : S ? () => ej(1) : () => ej(0),
                                    disabled: ed || ey > 0,
                                    className: `bg-green-600 text-white text-sm font-medium px-4 py-2 rounded-md transition-colors duration-200 ${ed || ey > 0 ? "opacity-50 cursor-not-allowed" : "hover:bg-green-700 hover:cursor-pointer"}`,
                                    children: ed ? `${r("checking")}...` : ey > 0 ? `${ey}s` : r(S ? "resendOtp" : "lblInputSentOtp")
                                })]
                            })]
                        }), I && (0,
                        t.jsx)("div", {
                            className: "mb-2",
                            children: (0,
                            t.jsxs)("p", {
                                className: "text-[10px] font-light text-green-500",
                                children: [r("payPhone"), " ", r("verified"), " "]
                            })
                        }), S && !I && (0,
                        t.jsxs)("div", {
                            className: "mb-2",
                            children: [(0,
                            t.jsx)("label", {
                                className: "block text-sm font-medium text-gray-700 mb-2",
                                children: r("verifyOtp")
                            }), (0,
                            t.jsxs)("div", {
                                className: "flex items-center gap-3 rounded-md md:flex-nowrap flex-wrap",
                                children: [(0,
                                t.jsx)("input", {
                                    type: "tel",
                                    value: A,
                                    onChange: e => C(e.target.value),
                                    className: "flex-1 px-3 py-2 text-sm rounded-md border border-gray-300 bg-gray-100 text-gray-700 focus:outline-none",
                                    placeholder: r("lblInputOtp"),
                                    autoComplete: "off"
                                }), (0,
                                t.jsx)("button", {
                                    onClick: A.length >= 6 && !eu && p ? ew : void 0,
                                    disabled: A.length < 6 || eu || !p,
                                    className: `bg-green-600 text-white text-sm font-medium px-4 py-2 rounded-md transition-colors duration-300 ${A.length < 6 || eu || !p ? "opacity-50 cursor-not-allowed" : "hover:bg-green-700 hover:cursor-pointer"}`,
                                    children: eu ? r("checking") + "..." : r("verify")
                                })]
                            })]
                        }), I && (0,
                        t.jsxs)("div", {
                            className: "my-4",
                            children: [(0,
                            t.jsx)("label", {
                                className: "block text-sm font-medium text-gray-700 mb-2",
                                children: r("lblInputAppointDate")
                            }), (0,
                            t.jsxs)("select", {
                                id: "appointment_date",
                                name: "appointment_date",
                                value: H ?? "",
                                onChange: eN,
                                className: "block w-full rounded-md px-3 py-1.5 text-base font-light text-black outline-1 -outline-offset-1 outline-gray-300 placeholder:text-gray-400 sm:text-sm/6",
                                children: [(0,
                                t.jsx)("option", {
                                    value: "",
                                    disabled: !0,
                                    hidden: !0,
                                    children: r("lblInputAppointment")
                                }), $ && Object.keys($).length > 0 && Object.entries($).map( ([e,a]) => (0,
                                t.jsx)("option", {
                                    value: a,
                                    children: a
                                }, e))]
                            })]
                        }), K && (0,
                        t.jsxs)("div", {
                            className: "mb-6 mt-2",
                            children: [(0,
                            t.jsx)("label", {
                                className: "block text-sm font-medium text-gray-700 mb-2",
                                children: r("lblInputAppointTime")
                            }), (0,
                            t.jsxs)("select", {
                                id: "appointment_date",
                                name: "appointment_date",
                                value: J ?? "",
                                onChange: e => Y(e.target.value),
                                className: "block w-full rounded-md px-3 py-1.5 text-base font-light text-black outline-1 -outline-offset-1 outline-gray-300 placeholder:text-gray-400 sm:text-sm/6",
                                children: [(0,
                                t.jsx)("option", {
                                    value: "",
                                    disabled: !0,
                                    hidden: !0,
                                    children: r("selectAppointmentTime")
                                }), Array.isArray(G) && G.length > 0 && G.map(e => (0,
                                t.jsx)("option", {
                                    value: e.time_display,
                                    children: e.time_display
                                }, e.id))]
                            })]
                        }), "" !== J && (0,
                        t.jsx)(t.Fragment, {
                            children: (0,
                            t.jsxs)("div", {
                                className: "flex flex-col justify-between items-center gap-4 mb-6 mt-2",
                                children: [(0,
                                t.jsxs)("div", {
                                    className: "flex",
                                    children: [(0,
                                    t.jsx)("img", {
                                        src: z,
                                        alt: "Captcha",
                                        className: "w-full h-10 object-contain"
                                    }), (0,
                                    t.jsx)("div", {
                                        className: "ml-2 mt-1 hover:cursor-pointer hover:bg-blue-200 hover:rounded-md p-1",
                                        onClick: () => eg(),
                                        children: !ea && (0,
                                        t.jsx)(D.RefreshCw, {
                                            size: 20
                                        })
                                    })]
                                }), (0,
                                t.jsxs)("div", {
                                    className: "w-full flex items-center gap-2",
                                    children: [(0,
                                    t.jsx)("input", {
                                        type: "text",
                                        value: ee,
                                        maxLength: 6,
                                        disabled: ea,
                                        autoComplete: "off",
                                        onChange: e => et(e.target.value),
                                        className: "w-full h-10 px-3 py-2 text-sm rounded-md border border-gray-300 bg-gray-100 text-gray-700 focus:outline-none",
                                        placeholder: r("enterCaptcha")
                                    }), (0,
                                    t.jsx)("button", {
                                        type: "button",
                                        onClick: es || ea ? void 0 : e_,
                                        disabled: 6 !== ee.length || es || ea,
                                        className: `h-10 px-4 text-sm font-medium rounded-md text-white transition-colors duration-300 ${6 === ee.length && !ea && !es ? "bg-green-600 hover:bg-green-700 hover:cursor-pointer" : "bg-gray-400 cursor-not-allowed"}`,
                                        children: es ? r("checking") + "..." : r(ea ? "verified" : "verify")
                                    })]
                                }), (0,
                                t.jsx)("div", {
                                    className: "w-full flex items-center gap-2",
                                    children: "" !== en && (0,
                                    t.jsx)("p", {
                                        className: "text-xs text-red-500 mt-0",
                                        children: en
                                    })
                                })]
                            })
                        }), (0,
                        t.jsx)("button", {
                            className: `w-full py-3 mb-2 rounded-lg duration-300 text-white
    ${eA && !eh ? "bg-green-600 hover:bg-green-700 cursor-pointer" : "bg-gray-400 cursor-not-allowed"}`,
                            disabled: !eA || eh,
                            onClick: eA && !eh ? eC : void 0,
                            children: eh ? r("processing") + "..." : r("payNow")
                        }), (0,
                        t.jsx)("p", {
                            className: "text-xs text-red-500 text-center",
                            children: r("msgTrans5min")
                        })]
                    })]
                })
            })]
        })
    }
    ;
    e.s(["default", 0, () => {
        let e = (0,
        l.useTranslations)("root")
          , i = (0,
        s.useSelector)(e => e.applicationStep.step)
          , n = [{
            id: 1,
            name: e("lblTabMenu1")
        }, {
            id: 2,
            name: e("lblTabMenu2")
        }, {
            id: 3,
            name: e("lblTabMenu3")
        }, {
            id: 4,
            name: e("lblTabMenu4")
        }]
          , r = {
            hidden: {
                opacity: 0,
                x: 20
            },
            visible: e => ({
                opacity: 1,
                x: 0,
                transition: {
                    delay: .3 * e
                }
            })
        };
        return (0,
        t.jsx)(t.Fragment, {
            children: (0,
            t.jsxs)(a.motion.div, {
                className: "flex flex-col bg-[#b5d6a9] sm:mx-8 lg:mx-20 border-[0.1px] border-slate-300 rounded-md shadow-lg pb-4",
                variants: {
                    hidden: {
                        opacity: 0,
                        y: 40
                    },
                    visible: {
                        opacity: 1,
                        y: 0,
                        transition: {
                            duration: .6,
                            ease: "easeOut"
                        }
                    }
                },
                initial: "hidden",
                animate: "visible",
                children: [(0,
                t.jsx)("div", {
                    className: "flex flex-row gap-4  text-xs overflow-x-scroll scrollbar-hide border-b-[0.1px] border-slate-300",
                    children: n.map( (e, l) => (0,
                    t.jsxs)(a.motion.p, {
                        custom: l,
                        variants: r,
                        initial: "hidden",
                        animate: "visible",
                        className: `py-2 px-4 flex items-center font-bold rounded-t min-w-[160px] ${i === e.id ? "bg-white text-black" : "text-[#5c7a2f]"}`,
                        children: [(0,
                        t.jsx)("span", {
                            className: `rounded-full px-2 py-1 mr-2 font-light text-xs italic ${i === e.id ? "bg-green-600 text-white" : "bg-white text-gray-500"}`,
                            children: e.id
                        }), e.name]
                    }, e.id))
                }), (0,
                t.jsxs)("div", {
                    className: "bg-white w-full p-4 overflow-y-scroll",
                    children: [1 === i && (0,
                    t.jsx)(j, {}), 2 === i && (0,
                    t.jsx)(S, {}), 3 === i && (0,
                    t.jsx)(I, {}), 4 === i && (0,
                    t.jsx)(U, {})]
                })]
            })
        })
    }
    ], 23653)
}
]);
